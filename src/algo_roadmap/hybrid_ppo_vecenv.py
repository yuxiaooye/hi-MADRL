...
class VecHybridPPO(Hybrid_PPO_base):
    ...
    def build_meta_gradient_and_update_svo(self, i, input_dict, new_actor, old_actor):
        ...
        if self.use_copo1:
            ...
            advantages = torch.sin(svo_rad) * input_dict['advantage'] + torch.cos(svo_rad) * input_dict['nei_advantage']
        if self.use_copo2:
            if self.hcopo_shift:
            ...
            if self.hcopo_sqrt2_scale:
                nei = torch.sqrt(torch.tensor(2.0)) * torch.sin(theta_rad) * input_dict['uav_advantage'] + torch.cos(theta_rad) * input_dict['car_advantage']
            else:
                nei = torch.sin(theta_rad) * input_dict['uav_advantage'] + torch.cos(theta_rad) * input_dict['car_advantage']
            advantages = torch.sin(phi_rad) * input_dict['advantage'] + torch.cos(phi_rad) * nei  # 4_26 11:30 ddlbug
